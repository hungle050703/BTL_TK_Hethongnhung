...
// Copilot: Clean this file by removing all demo/test code.
// Keep only initialization functions (LCD_Init, LCD_Config),
// low-level draw APIs (LCD_DrawPixel, LCD_FillRect, LCD_DrawLine, etc),
// and necessary hardware setup.
// Do not remove any code related to hardware configuration or SPI/FSMC.
// Copilot: Functions to KEEP:
//   LCD_Init(), LCD_Reset(), LCD_Config(), LCD_DrawPixel(), LCD_SetAddressWindow()
//   LCD_WriteCommand(), LCD_WriteData(), LCD_FillRect(), LCD_Clear()
// Copilot: Functions to DELETE:
//   LCD_Test(), LCD_ShowFPS(), LCD_DrawDemo(), LCD_ShowImage(), LCD_UGUI_Demo()
...
const uint8_t init_cmd[] = {
    ...
    /* Use literal 0xB2 for PORCTRL to avoid header dependency when reusing driver */
    5,  0xB2,       0x0C, 0x0C, 0x00, 0x33, 0x33,   // PORCTRL: Standard porch
...
static void setSPI_Size(int8_t size){
  if(config.spi_sz != size){
    ...
    config.spi_sz = size;
    LCD_HANDLE.Init.DataSize = (size == mode_16bit) ? SPI_DATASIZE_16BIT : SPI_DATASIZE_8BIT;
    if (HAL_SPI_Init(&LCD_HANDLE) != HAL_OK) {
      Error_Handler();
    }
  }
}
